...
const generateServerWithMockFiles = async (files = []) => {
	return Promise.all(
		...
		res.map((fileContent) => {
			Object.entries(
				(fileContent && fileContent.default && fileContent.default.default) ||
				...
				if (cacheUrlList.includes(reqKey)) {
					return logger.warn(
						chalk.yellow(`The repeated Mock proxy, URL is：${reqKey}`)
					);
				}
...
const createRequest = (opt) => {
	...
	app[method.toLocaleLowerCase()](url, function (req, res, next) {
		res.set("Connection", "close");
		// cacheResList.push(res)
		if (!cacheResMap.get(url)) {
			cacheResMap.set(url, res);
		}
...
const initExpress = async ({ port, host, insertWatcher }) => {
	...
	if (!insertWatcher) {
		...
		server.listen(port, () => {
			logger.info(
				`${chalk.hex("#10a8cd")("Mock server is Ready：")}${chalk.green.bold(
					`http://${host}:${port}`
				)}`
			);
		});
...
const initMockServer = (options) => {
	...
	if (watch) {
		watcher(`${getBasePath()}mock/`, (eventType, filename) => {
			// for (let resmap of cacheResMap) {
			//   // resmap[1].set('Connection', 'close')
			//   resmap[1].end()
			//   console.log('new-res', resmap[0],resmap[1].get('Connection'))
			// }
